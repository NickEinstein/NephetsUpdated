...
const DeliveryLog = () => {
  ...
  const getDeliveryLogResult = DeliveryApi.useGetAllDeliveriesQuery({
    // RackId: formData.store,
    // Location,
  });
  const DeliveryLog = getDeliveryLogResult?.data;
  const sortedData = DeliveryLog?.slice()?.sort(compareDatetime);

  const [searchTerm, setSearchTerm] = useState("");
  const [filteredVendors, setFilteredVendors] = useState([]);

  const handleSearch = (tt) => {
    console.log(tt);
    if (true) {
      const filteredList = sortedData.filter((vendor) =>
        vendor.deliveryItemCode.toLowerCase().includes(tt.toLowerCase())
      );
          ...
          {/* <div>
            ...
            <ul>
              {filteredVendors.map((vendor) => (
                <li key={vendor.id}>{vendor.deliveryItemCode}</li>
              ))}
            </ul>
          </div> */}
